...
onMounted(() => {
    if (props.service.image) {
        previewImage.value = `/storage/${props.service.image}`
    }

    if (props.service.icon) {
        previewIcon.value = `/storage/${props.service.icon}`
    }
})
onBeforeUnmount(() => {
    cropper?.destroy()
        ...
        <template #Toolbar>
            <div class="toolbar mb-5 mb-lg-7">
                ...
                <div>
                    ...
                    <button class="btn btn-dark" @click="save" :disabled="form.processing">
                        Save
                    </button>
                </div>
            </div>
